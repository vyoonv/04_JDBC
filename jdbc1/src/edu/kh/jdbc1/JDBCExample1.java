...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3. SQL 작성 
			// ** JAVA 에서 작성되는 SQL은 ""마지막에 ; 작성하면 안됨 **
			...
			// 5. 생성된 Statement객체에 
			// sql을 적재하여 실행한 후 결과를 반환받아와 rs 변수에 저장 
			rs = stmt.executeQuery(sql);
			// executeQuery() : SELECT문 수행 메서드, ResultSet 반환 
			
			// 3단계 : SQL 수행해서 반환받은 결과(ResultSet)를 
			//         한 행씩 접근해서 컬럼값 얻어오기 
			...
			while(rs.next()) {
				...
				// java.sql.Date 
				Date hireDate = rs.getDate("HIRE_DATE"); // YYYY-MM-DD 19990-02-06
